...
const QuestionCard = ({ item, index }) => {
    ...
    return (
        <div className="bg-[#1c2230] border border-[#2a3348] rounded-[0.6rem] overflow-hidden transition-colors duration-200 hover:border-[#3a4562]">
            <div
                className="flex items-start gap-3 py-[0.9rem] px-4 cursor-pointer select-none"
                onClick={() => setOpen(o => !o)}
            >
                <span className="flex-shrink-0 text-[0.7rem] font-bold text-[#ff2d78] bg-[#ff2d78]/10 border border-[#ff2d78]/20 rounded-[0.3rem] py-[0.15rem] px-[0.4rem] mt-0.5">
                    Q{index + 1}
                </span>
                <p className="flex-1 m-0 text-[0.9rem] font-medium text-[#e6edf3] leading-[1.5]">
                    {item.question}
                </p>
                <span
                    className={`flex-shrink-0 text-[#7d8590] transition-transform duration-200 mt-0.5 ${
            ...
            {open && (
                <div className="px-4 pb-4 pt-3 flex flex-col gap-3 border-t border-[#2a3348]">
                    <div className="flex flex-col gap-1.5">
                        <span className="text-[0.68rem] font-bold uppercase tracking-[0.06em] py-[0.15rem] px-2 rounded w-fit text-[#a78bfa] bg-[#a78bfa]/10 border border-[#a78bfa]/20">
                            Intention
                        </span>
                        <p className="m-0 text-[0.835rem] text-[#9aa1ab] leading-[1.6]">{item.intention}</p>
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <span className="text-[0.68rem] font-bold uppercase tracking-[0.06em] py-[0.15rem] px-2 rounded w-fit text-[#3fb950] bg-[#3fb950]/10 border border-[#3fb950]/20">
                            Model Answer
                        </span>
                        <p className="m-0 text-[0.835rem] text-[#9aa1ab] leading-[1.6]">{item.answer}</p>
                    </div>
                </div>
            )}
        </div>
...
const InterviewReport = () => {
      ...
      return (
                <div className="w-full min-h-screen bg-[#0d1117] text-[#e6edf3] font-sans flex items-stretch p-6 box-border">
                          ...
                          {NAV_ITEMS.map(item => {
                              ...
                              return (
                                  <button
                                      ...
                                      className={`flex items-center gap-2.5 w-full py-2.5 px-3 bg-transparent border-none rounded-lg font-sans text-sm cursor-pointer text-left transition-colors duration-150 ${
                                          isActive
                  ...
                  {/* ── Center Content ── */}
                  <main className="flex-1 py-7 px-8 overflow-y-auto max-h-[calc(100vh-3rem)] pb-20 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      {activeNav === 'technical' && (
                          <section>
                  ...
                  {/* ── Right Sidebar ── */}
                  <aside className="w-60 flex-shrink-0 py-7 px-5 flex flex-col gap-5">
                      ...
                      <div className="flex flex-col items-center gap-2.5">
                          ...
                          <div className={`w-[90px] h-[90px] rounded-full flex flex-col items-center justify-center border-4 ${scoreColorClass}`}>
                          ...
                          <p className="m-0 text-xs text-[#3fb950] text-center">Strong match for this role</p>
                      </div>
                      ...
                      {/* Skill Gaps */}
                      <div className="flex flex-col gap-3">
                          <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#7d8590] m-0">Skill Gaps</p>
                          <div className="flex flex-wrap gap-2">
                              {report.skillGaps.map((gap, i) => {
                                  ...
                                  return (
                                      <span key={i} className={`text-[0.775rem] font-medium py-[0.3rem] px-[0.7rem] rounded-md border cursor-default ${sev}`}>
                                          {gap.skill}
                                      </span>
                                  )
                              })}
